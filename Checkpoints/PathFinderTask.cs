...
namespace RoutePlanning
{
    public static class PathFinderTask
    {
        public static int[] FindBestCheckpointsOrder(Point[] checkpoints)
        {
            ...
            MakePermutations(new int[checkpoints.Length], 1, result, checkpoints, minValue);
            minValue[0] = int.MaxValue;
            return result[result.Count - 1];
        }

        static void MakePermutations(int[] permutation, int position, List<int[]> result,
                                       Point[] checkpoints, double[] minValue)
        {
            ...
            else
            {
                for (int i = 1; i < permutation.Length; i++)
                {
                    ...
                    if (PointExtensions.GetPathLength(checkpoints, permutation.Take(position + 1).ToArray())
                            >= minValue[0]) continue;
